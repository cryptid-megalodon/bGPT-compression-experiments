...
class SimpleCPUSimulator:
    ...
    def _execute(self, op, addr1, addr2, addr3):
        ...
        if op == self.map_instruction["HLT"]:
            ...
            if addr1 == 0:
                ...
                self.registers[addr1 - 1] = (
                    (self.registers[addr1 - 1] << 1) & 0xFF
                ) | ((self.registers[addr1 - 1] >> 7) & 0xFF)
            ...
            if addr2 == 0:
                ...
                self.registers[addr1 - 1] = min(
                    self.registers[addr1 - 1] + self.registers[addr2 - 1], 255
                )
            else:
                self.registers[addr3 - 1] = min(
                    self.registers[addr1 - 1] + self.registers[addr2 - 1], 255
                )
            ...
            if addr2 == 0:
                ...
                self.registers[addr1 - 1] = max(
                    self.registers[addr1 - 1] - self.registers[addr2 - 1], 0
                )
            else:
                self.registers[addr3 - 1] = max(
                    self.registers[addr1 - 1] - self.registers[addr2 - 1], 0
                )
    ...
    def back_translate_program(self, program):
        ...
        program = [
            self.back_translate(program[i : i + 4]) for i in range(0, len(program), 4)
        ]
    ...
    def back_translate(self, instruction):
        ...
        op = list(self.map_instruction.keys())[
            list(self.map_instruction.values()).index(op)
        ]

        if addr1 != 0 and op != "LOADI":
            addr1 = list(self.map_register.keys())[
                list(self.map_register.values()).index(addr1)
            ]
        elif op == "LOADI":
            addr1 = str(addr1)
        else:
        ...
        if addr2 != 0:
            addr2 = list(self.map_register.keys())[
                list(self.map_register.values()).index(addr2)
            ]
        else:
            addr2 = ""

        if addr3 != 0:
            addr3 = list(self.map_register.keys())[
                list(self.map_register.values()).index(addr3)
            ]
        else:
            addr3 = ""
            ...
            if instruction == "LOADI":
                ...
                if addr_num == 0:
                    ...
                    program.append(
                        instruction
                        + " "
                        + random.choice(list(self.map_register.keys()))
                    )
                elif addr_num == 2:
                    program.append(
                        instruction
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="generate",
        help="Mode of the CPU simulation. Options: 'generate', 'translate', and 'evaluate'",
    )
    parser.add_argument(
        "--num_registers", type=int, default=10, help="Number of registers in the CPU"
    )
    parser.add_argument(
        "--memory_size", type=int, default=1024, help="Size of the memory in the CPU"
    )
    parser.add_argument(
        "--dir_path",
        type=str,
        default="cpu_states",
        help="Path to generate or evaluate the CPU states, only for 'generate' and 'evaluate' modes",
    )
    parser.add_argument(
        "--states_path",
        type=str,
        default="cpu_states/cpu.bin",
        help="Path to the CPU states, only for 'translate' mode",
    )
    parser.add_argument(
        "--num_min_program",
        type=int,
        default=1,
        help="Minimum number of instructions in the program, only for 'generate' mode",
    )
    parser.add_argument(
        "--num_max_program",
        type=int,
        default=255,
        help="Maximum number of instructions in the program, only for 'generate' mode",
    )
    parser.add_argument(
        "--num_train_instance",
        type=int,
        default=2100000,
        help="Number of training instances, only for 'generate' mode",
    )
    parser.add_argument(
        "--num_test_instance",
        type=int,
        default=21000,
        help="Number of testing instances, only for 'generate' mode",
    )
    parser.add_argument("--random_seed", type=int, default=0, help="Random seed")
    parser.add_argument(
        "--output_path",
        type=str,
        default="cpu_states.txt",
        help="Path to save the translated CPU states, only for 'translate' mode",
    )
...
if __name__ == "__main__":
    ...
    if mode == "generate":
        ...
        print(
            "Generating the CPU States dataset with %d training instances and %d testing instances at %s"
            % (num_train_instance, num_test_instance, dir_path)
        )

        for _ in tqdm(range(num_train_instance)):
            cpu = SimpleCPUSimulator(
                num_registers=num_registers, memory_size=memory_size, random_init=True
            )
        ...
        cpu = SimpleCPUSimulator(num_registers=num_registers, memory_size=memory_size)
