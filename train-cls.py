...
model = utils.bGPTForClassification(patch_config, len(train_set.labels))
...
def train_epoch():
    ...
    for batch in tqdm_train_set:
        ...
        tqdm_train_set.set_postfix(
            {
                str(global_rank) + "_train_acc": total_acc_num
                / (iter_idx * cfg.BATCH_SIZE)
            }
        )
        if iter_idx % 20 == 0:
...
def eval_epoch():
    ...
    for batch in tqdm_eval_set:
        ...
        tqdm_eval_set.set_postfix(
            {
                str(global_rank) + "_eval_acc": total_acc_num
                / (iter_idx * cfg.BATCH_SIZE)
            }
        )
        iter_idx += 1
...
if __name__ == "__main__":
    ...
    eval_sampler = DistributedSampler(
        eval_set, num_replicas=world_size, rank=global_rank
    )
    ...
    if cfg.LOAD_FROM_PRETRAINED:
        ...
        pretrained_model = utils.bGPTLMHeadModel(patch_config, byte_config)
        ...
        if torch.cuda.device_count() > 1:
            ...
            cpu_model.patch_level_decoder.load_state_dict(
                pretrained_model.patch_level_decoder.state_dict()
            )
        ...
        try:
            ...
            print(
                f"Successfully Loaded Pretrained Checkpoint at Epoch {checkpoint['epoch']} with Acc {checkpoint['max_eval_acc']}"
            )
