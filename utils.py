...
import torch
from config import *
from samplings import temperature_sampling, top_k_sampling, top_p_sampling
from transformers import GPT2LMHeadModel, GPT2Model, PreTrainedModel
...
class PatchLevelDecoder(PreTrainedModel):
    ...
    def forward(self, patches: torch.Tensor, masks=None) -> torch.Tensor:
        ...
        """
        patches = torch.nn.functional.one_hot(patches, num_classes=256 + 1).to(
            self.dtype
        )
...
class ByteLevelDecoder(PreTrainedModel):
    ...
    def forward(self, encoded_patches: torch.Tensor, target_patches: torch.Tensor):
        ...
        target_patches = torch.cat(
            (
                torch.ones_like(target_patches[:, 0:1]) * self.special_token_id,
                target_patches,
            ),
            dim=1,
        )
        ...
        return self.base(inputs_embeds=inputs_embeds, labels=target_patches)
...
class bGPTLMHeadModel(PreTrainedModel):
    ...
    def generate(self, patches: torch.Tensor, top_k=0, top_p=1, temperature=1.0):
        ...
        if patches.shape[-1] % PATCH_SIZE != 0:
            ...
            tokens = torch.cat(
                (torch.tensor([self.special_token_id], device=self.device), tokens),
                dim=-1,
            )
        ...
        while True:
            prob = (
                self.byte_level_decoder.generate(encoded_patches[0][-1], tokens)
                .cpu()
                .detach()
                .numpy()
            )
